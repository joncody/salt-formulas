...
More specifically, [custom.bashrc](https://github.com/joncody/salt-formulas/blob/master/bashrc/files/custom.bashrc) appends to `/etc/bash.bashrc` and tells the machine to update `PATH`, `LIBRARY_PATH`, `LD_LIBRARY_PATH`, `CPATH`, and `PKG_CONFIG_PATH` by searching `/opt`.
